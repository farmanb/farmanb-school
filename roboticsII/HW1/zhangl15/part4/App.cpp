...
App::~App() { 
  ...
  for(int i=0; i<5; i++)
  {
    if(goalController[i]) delete goalController[i];
  }
...
//check for the goal.
  if (!GetGoal()){
    printf("App::FlushAppSettings(): Error!  Can not find one or more in %s, %s, %s, %s and %s in simulation!\n", goalBack->c_str(),goalFront->c_str(),goalSide1->c_str(),goalSide2->c_str(), goalBottom->c_str() );
    assert(GetGoal());
  }
...
bool App::AddGoalController(){
	...
	goalBody[0] = goalBackBody;
	goalBody[1] = goalFrontBody;
	goalBody[2] = goalSide1Body;
	goalBody[3] = goalSide2Body;
	goalBody[4] = goalBottomBody; //save pointers to goal components in a pointer array

        sim->GetBodyByNameMutable(*ballName, ballBody);	
	for(int i=0; i<5; i++)
	{
		if(goalController[i]) delete goalController[i]; 
		sim->RemoveAllBodyControllersByName(*(goalName[i]));
	} //delete old controllers

	DVC::Vector<REAL> goalPos;          //get initial position and quaternion defined in xml file
	DVC::Quaternion goalQuat;
	for(int i=0; i<5; i++)
	{
		goalPos = goalBody[i]->GetPos();
		goalQuat = goalBody[i]->GetQuat();
		goalController[i] = new GoalController(goalBody[i], goalPos, goalQuat, i, ballBody);      //initialize new controller for every part of the goal
		if(! sim->SetBodyControllerByName(*(goalName[i]), *(goalController[i]))) return false;
	}
	return true;
}
