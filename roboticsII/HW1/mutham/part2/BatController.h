...
class BatController : public PositionController {
 ...
 public: 
  ...
  BatController(KinematicalBody * body, REAL x, REAL y, 
		REAL z, DVC::Quaternion quat = DVC::Quaternion(0, 0, cos((pi)/2), sin((pi)/2))) : PositionController(body){  
    m_x = x;
    m_y = y;
    m_z = z;
    m_quat = quat;
  ...
  void GetConfiguration(REAL t, DVC::Vector<REAL> & outPos, DVC::Quaternion & outQuat) const{
    ...
    outPos(1) = m_y;
    outPos(2) = m_z;
    outQuat = DVC::Quaternion( 0, 0, cos((pi +(90*t))/2),sin((pi + (90*t))/2));
  }
