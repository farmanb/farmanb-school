...
Working within the homotopy category of the 2-category of all small dg-categories over a commutative ring, To\"en is able to provide an incredibly elegant reformulation of Fourier-Mukai functors at the level of pre-triangulated dg-categories via the internal Hom, $\mathbb{R}\underline{\operatorname{Hom}}_c$.
%the homotopy category's internal Hom.
...
In each case, the real work lies in identifying the results of this machinery within the theory from which the input originates, for even if the input arises geometrically, the results are often quite abstract.
...
In the situation of interest, one considers the noncommutative projective scheme, $\operatorname{QGr} A$, associated to the connected graded algebra, $A$, over a field, $k$, that are both left and right Noetherian.
The natural choice of dg-category is the dg-enhancement, $\mathcal{D}(\operatorname{QGr} A)$, of the derived category $\operatorname{D}(\operatorname{QGr} A)$, in the sense of \cite{Lunts-Orlov}.
The advantage of this choice is that this dg-category is unique up to equivalence in the homotopy category of dg-categories.
The work then lies in identifying the dg-category $\mathbb{R}\underline{\operatorname{Hom}}(\mathcal{D}(\operatorname{QGr} A), \mathcal{D}(\operatorname{QGr} B))$ as a noncommutative geometric object.%, and is established for rings satisfying some cohomological conditions by the author in recent joint work with his advisor \cite{BF17}.

Generally, care must be taken to ensure good behavior of $X$, but one may exert some control by imposing cohomological conditions on the ring, $A$.
...
Imposing these conditions on the rings $A$ and $B$, the author, in recent joint work with his advisor, establishes the identification
$$\mathbb{R}\underline{\operatorname{Hom}}(\mathcal{D}(\operatorname{QGr} A), \mathcal{D}(\operatorname{QGr} B)) \cong \mathcal{D}(\operatorname{QGr}(A^{\operatorname{op}} \otimes_k B))$$
and produces Fourier-Mukai kernels \cite{BF17}.
%the dg-enhancement of the derived category of quasi-coherent sheaves on the (bi)projective scheme associated to the $\mathbb{Z}^2$-graded $k$-algebra $A^{\operatorname{op}} \otimes_k B$, and produces Fourier-Mukai kernels \cite{BF17}.
...
This project will be successful if it advances our understanding of mathematics as laid out above.  This can be tangibly measured by production of preprints submitted to the arXiv and peer-reviewed journal publications, as well as talks given to the broader mathematical community.
