...
\begin{ex}
  ...
  \begin{proof}
    ...
    \item
      Note that the same characterization holds for $\operatorname{Tot}^{\oplus}(C)$, but we require that all but finitely many of the elements are zero in a tuple.
      By the same analysis, mutatis mutandis, we observe that the elements of $Z_0(\operatorname{Tot}^{\oplus}(C))$ are the elements $(\ldots, a_{-2,2}, a_{-1,1}, a_{0,0})$ with each of the $a_{p,-p}$ congruent modulo $2$ and that $B_0(\operatorname{Tot}^{\oplus}(C)) \cong \oplus_{p \leq 0}2\Z/4\Z$.
      However, if $a_{0,0} \equiv 1 \pmod{2}$, then it follows that $a_{p,-p} \neq 0$ for each $p \leq 0$, which cannot be an element of the direct sum.
      Therefore $H_0(\operatorname{Tot}^{\oplus}(C)) = 0$.
      Since the proof does not depend on the indices, by replacing $1$ with $n+1$, $0$ with $n$, and $-1$ with $n-1$, it follows that $\operatorname{Tot}^{\oplus}(C)$ is acyclic.
    \end{enumerate}
  \end{proof}
\end{ex}
...
\begin{ex}
  ...
  is exact, where $\ ^*$ denotes the Pontrjagin dual, $\Hom{\Ab}{\ , \Q/\Z}$.
  ...
  \begin{proof}
    ...
    But then we have
    $$f^*g^*(\tilde{h})(a) = f^*(\tilde{h} \circ g)(a) = \tilde{h} \circ g \circ f(\alpha) \neq 0,$$
    ...
    $$h(b + \im{f}) = h \circ \pi(b) = g^*(h^\prime) = h^\prime \circ g(b) = 0,$$
    a contradiction.
...
\begin{lem}\label{lem13}
  ...
  \begin{proof}
    Let $x_1, \ldots, x_n$ generate $M^\prime$,  $y_1, \ldots, y_m$ generate $M^{\prime\prime}$, and let $k_i = f(x_i)$.
    Since $g$ is surjective, there exist $z_i \in M$ such that $g(z_i) = y_i$.
    For any $m \in M$, there exist $r_i \in R$ such that
    $$g(m) = \sum r_iy_i = \sum r_ig(z_i) = \sum g(r_iz_i)$$
    and thus $m - \sum r_iz_i \in \ker g = \im f$.
    Choose $s_i \in R$ such that
    $$m - \sum r_iz_i = f\left(\sum s_i x_i\right) =  \sum s_ik_i$$
    and so it follows that
    $$m = \sum r_iz_i + \sum s_ik_i.$$
    Therefore $M$ is generated by $k_1, \ldots, k_n, z_1, \ldots, z_m$.
  \end{proof}
\end{lem}
