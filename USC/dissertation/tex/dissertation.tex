...
\begin{eg}
  ...
  \item[dg $k$-modules]
    ...
    $$\begin{tikzcd}
      \mathcal{C}_\text{dg}(k)(C^\bullet, D^\bullet)^m \arrow{r} & \mathcal{C}_\text{dg}(k)(C^\bullet, D^\bullet)^{m+1}\\
...
\begin{defn}
  ...
  \end{enumerate}
  Note that given a dg functor $\mathscr{F} \colon \CC \rightarrow \D$, for $X$ and $Y$ objects of $\CC$, $\mathscr{F}(X,Y) \colon \CC(X,Y) \rightarrow \D(\mathscr{F}X, \mathscr{F}Y)$ is a morphism of $\mathcal{C}(k)$.
  Hence $Z^0$ and $H^0$ induce functors 
  $$\begin{tikzcd}
    Z^0(\mathscr{F}) \colon Z^0(\CC) \arrow{r}& Z^0(\D)
  \end{tikzcd}
  \text{and}
  \begin{tikzcd}
    H^0(\mathscr{F}) \colon H^0(\CC) \arrow{r}& H^0(\D)
  \end{tikzcd}$$
  in the obvious way.
\end{defn}
