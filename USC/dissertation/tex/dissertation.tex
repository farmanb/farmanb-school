...
\begin{defn}
  ...
  \begin{enumerate}
  \item
    if each morphism $f : X \rightarrow Y$ of $Z^0(\mathscr{C})$ admits a morphism $Y \rightarrow C(f)$ of $Z^0(\mathscr{C})$
  \item
    for each object $X$ of $Z^0(\mathscr{C})$, there is an isomorphism $h_X[n] \cong h_{X[n]}$, and
  \item
    the cone in $\mathcal{C}(\mathscr{C})$ of the morphism $h_X(f)$ is the image of the morphism $Y \rightarrow C(f)$.
  \end{enumerate}
\end{defn}
...
\begin{defn}
  Let $\mathscr{C}$ be a small, pretriangulated dg category admitting coproducts, let $E$ be an object of $\mathscr{C}$, and let $\mathscr{T} = H^0(\mathscr{C})$.
  By abuse of notation, we say
  \begin{enumerate}
  \item
    $E$ is a {\it classical generator} of $\mathscr{C}$ if $E$ is a classical generator of $\mathscr{T}$,
  \item
    $E$ is a {\it strong generator} of $\mathscr{C}$ if $E$ is a strong generator of $\mathscr{T}$, and
  \item
    $E$ is a {\it (weak) generator} of $\mathscr{C}$ if $E$ is a (weak) generator of $\mathscr{T}$.
  \end{enumerate}
\end{defn}
