...
\begin{defn}
  ...
  Hence we are justified in defining the $\CC$-module $\cone{\eta} \colon Z^0(\CC) \to \mathcal{C}(k)$ by $\cone{\eta}(X) = \cone{\eta(X)}$ equipped with differential 
  $$d_{\cone{\eta}}^n = 
  \left(\begin{matrix}
    d_{\mathscr{L}[1]}^n & 0\\
    -\eta & d_{\mathscr{M}}^n
  \end{matrix}\right)$$
  on objects $X$ of $\CC$, and on morphisms $f \in Z^0(\CC(Y,X))$
  $$\begin{tikzcd}
    \mathscr{L}(X)[1]\arrow{rd}{\left(\begin{matrix}-1\\0\end{matrix}\right)}\arrow{dd}{\mathscr{L}(f)[1]} & & \mathscr{M}(X) \arrow[swap]{ld}{\left(\begin{matrix}0\\1\end{matrix}\right)}\arrow{dd}{\mathscr{M}(f)}\\
    & \cone{\eta}(X)\arrow[dashed]{dd}{\exists !\cone{\eta}(f)}\\
    \mathscr{L}(Y)[1]\arrow[swap]{rd}{\left(\begin{matrix}-1\\0\end{matrix}\right)} & & \mathscr{M}(Y)\arrow{ld}{\left(\begin{matrix}0\\1\end{matrix}\right)}\\
    & \cone{\eta}(Y)
  \end{tikzcd}$$
  By writing
  $$\cone{\eta}(f) = \left(\begin{matrix}
    \mathscr{L}(f)[1] & 0\\
    0 & \mathscr{M}(f)
  \end{matrix}\right)$$
  it's clear that this is indeed a morphism of complexes.
\end{defn}

\begin{rmk}
  We note that we have a short exact sequence of $\CC$-modules
  $$\begin{tikzcd}
