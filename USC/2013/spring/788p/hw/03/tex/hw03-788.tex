...
\begin{defn}[Limit]\label{def1}
  ...
  Since $\Delta c$ is the constant functor, the natural transformation $\tau$ is the collection of morphisms $\tau_j \colon c \rightarrow F_j$ of $\mathcal{C}$, for each object $j$ of $J$, such that for every arrow $u \colon j \rightarrow k$ of $J$, the diagram 
    \begin{center}
      \begin{tikzcd}
        & \arrow{ld}[description]{\tau_j}c\arrow{rd}[description]{\tau_j} & & j\arrow{d}[description]{u}\\
        F_j\arrow{rr}[description]{Fu} & & F_k & k
      \end{tikzcd}
    \end{center}
    %$\tau_k = Fu \circ \tau_j$
  ...
  \begin{center}
    \begin{tikzcd}
      & \arrow[bend right]{ldd}{\tau_j}\arrow[dotted]{d}{\exists !h} c\arrow[bend left]{rdd}{\tau_j} &\\
      & \arrow{ld}{\nu_j}\underleftarrow{\lim}F_i\arrow{rd}{\nu_j} &\\
      F_j\arrow{rr}{Fu} & & F_k
  \end{tikzcd}
\end{center}
  commutes.
