...
bool App::Init(){
  ...
  movestep=0;
  handController->StartScriptFile("hw2ascript.txt");

  //handController->HandInvKin(-.4,1.5,.26,0,1.5,-.26,-.45);
  ///handController->HandInvKin(pos);
...
void App::GoToPartCG(){
  ...
  for (unsigned int i=0; i<4; i++)
	...
	gripController[i]->SetPDTarget(0,2);
}
